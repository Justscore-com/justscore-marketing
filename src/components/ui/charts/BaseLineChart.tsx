...
interface EnhancedBaseLineChartProps extends BaseLineChartProps {
  ...
  dotSize?: number; // Customizable dot size
  activeDotSize?: number; // Customizable active dot size
  connectNulls?: boolean; // Connect lines through null data points
...
export function EnhancedBaseLineChart({
  data,
  xKey,
  yKeys,
  labelKeys,
  ...
  const sampleData = [
    { period: "Period 1", line1: 3.2, line2: 3.8, line3: 3.5 },
    { period: "Period 2", line1: 3.5, line2: 3.6, line3: 3.9 },
    { period: "Period 3", line1: 3.8, line2: 4.1, line3: 3.7 },
  ];
  ...
  const chartData = hasInsufficientData 
    ? sampleData.map(item => {
        ...
        yKeys.forEach((key, index) => {
          mapped[key] = item[`line${index + 1}` as keyof typeof item] || 0;
        });
        return mapped;
      })
    : data?.map(item => {
            ...
            <defs>
              {colors.map((color, index) => (
                <linearGradient
                  ...
                  id={`chart-gradient-${uniqueId}-${index}`}
                  x1="0%"
                  y1="0%"
                  x2="0%"
                  y2="100%"
                >
                  <stop 
                    offset="0%" 
                    stopColor={color} 
                    stopOpacity={hasInsufficientData ? 0.4 : 1} 
                  />
                  <stop 
                    offset="100%" 
                    stopColor={color} 
                    stopOpacity={hasInsufficientData ? 0.2 : 0.5} 
                  />
                </linearGradient>
              ))}
            </defs>
...
export { EnhancedBaseLineChart as BaseLineChart };